from psychopy import visual, core, event
import random
import csv
import datetime
import os

# Helper function for text input inside fullscreen
def get_text_input(win, prompt, allowed_chars, max_len=20):
    input_text = ''
    prompt_stim = visual.TextStim(win, text=prompt, color='black', height=40, pos=(0, 100))
    text_stim = visual.TextStim(win, text='', color='black', height=40, pos=(0, 0))
    instructions = visual.TextStim(win, text="Press ENTER to confirm, BACKSPACE to delete.", pos=(0, -100), color='gray', height=30)

    while True:
        prompt_stim.draw()
        text_stim.text = input_text
        text_stim.draw()
        instructions.draw()
        win.flip()

        keys = event.waitKeys()
        for key in keys:
            if key == 'return':
                return input_text.strip()
            elif key == 'backspace':
                input_text = input_text[:-1]
            elif key.lower() in allowed_chars and len(input_text) < max_len:
                input_text += key

def wait_for_key():
    event.waitKeys()

# === Set up fullscreen window ===
win = visual.Window(fullscr=True, color='white', units='pix')

# === Collect participant info ===
name = get_text_input(win, "Enter your name:", allowed_chars=list("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_ "))
group = get_text_input(win, "Enter group (1, 2, or 3):", allowed_chars=['1', '2', '3'])

participant_name = name.replace(" ", "_") or "anonymous"
timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
results_dir = "stroop_data"
os.makedirs(results_dir, exist_ok=True)
filename = os.path.join(results_dir, f"stroop_results_{participant_name}_G{group}_{timestamp}.csv")

# === Define stimuli ===
words = ['RED', 'BLUE', 'GREEN']
colors = {'red': [1, -1, -1], 'blue': [-1, -1, 1], 'green': [-1, 1, -1]}
key_map = {'r': 'red', 'g': 'green', 'b': 'blue'}

text_stim = visual.TextStim(win=win, text='', height=100, pos=(0, 0))
instruction = visual.TextStim(win=win, text="Press R for RED, G for GREEN, B for BLUE.\n\nPress ESC to quit at any time.\n\nPress any key to start.", color='black', height=40)

# Show instructions
instruction.draw()
win.flip()
event.clearEvents()
wait_for_key()

results = []

# === Part 1: Color Stroop ===
for trial in range(10):
    word = random.choice(words)
    color_name = random.choice(list(colors.keys()))
    color_rgb = colors[color_name]

    text_stim.text = word
    text_stim.color = color_rgb
    text_stim.pos = (0, 0)
    text_stim.draw()
    win.flip()

    print(f"Trial {trial + 1}: Word='{word}', Color='{color_name}'")

    start_time = core.getTime()
    keys = event.waitKeys(keyList=['r', 'g', 'b', 'escape'])
    rt = core.getTime() - start_time

    if 'escape' in keys:
        print("👋 Task exited early by user.")
        win.close()
        core.quit()

    response = key_map.get(keys[0].lower(), '')
    correct = (response == color_name)

    results.append([
        trial + 1, word, color_name, response, correct,
        round(rt * 1000), participant_name, group, timestamp
    ])

    feedback = visual.TextStim(win, text="Correct!" if correct else "Wrong!", color='black', height=40)
    feedback.draw()
    win.flip()
    core.wait(0.5)

# End screen for Part 1
end_message = visual.TextStim(win, text="Done! Thank you.\nPress any key to continue to Part 2.", color='black', height=40)
end_message.draw()
win.flip()
event.clearEvents()
wait_for_key()

# === Save results for Part 1 ===
with open(filename, 'w', newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow(['Trial', 'Word', 'Color_Shown', 'Response', 'Correct', 'Reaction_Time_ms', 'Participant', 'Group', 'Timestamp'])
    writer.writerows(results)

    total_trials = len(results)
    correct_trials = sum(1 for r in results if r[4])
    accuracy = (correct_trials / total_trials) * 100 if total_trials > 0 else 0
    avg_rt_correct = sum(r[5] for r in results if r[4]) / correct_trials if correct_trials > 0 else 0

    writer.writerow([])
    writer.writerow(['Summary'])
    writer.writerow(['Percent Accuracy (%)', f"{accuracy:.2f}"])
    writer.writerow(['Average RT (ms) Correct Answers', f"{avg_rt_correct:.0f}"])

print(f"✅ Results saved to {filename}")

# === Part 2: Spatial Stroop ===
text_stim.pos = (0, 0)  # Reset position
instruction.text = "PART 2: Spatial Stroop\n\nA word ('LEFT' or 'RIGHT') will appear on either the left or right side.\n\nPress 'T' if the word's meaning matches its position.\nPress 'F' if it does NOT match.\n\nPress any key to start."
instruction.draw()
win.flip()
event.clearEvents()
wait_for_key()

spatial_results = []
positions = {'left': (-300, 0), 'right': (300, 0)}
spatial_words = ['LEFT', 'RIGHT']

for trial in range(10):
    word = random.choice(spatial_words)
    position_name = random.choice(['left', 'right'])
    pos = positions[position_name]

    text_stim.text = word
    text_stim.color = 'black'
    text_stim.pos = pos
    text_stim.draw()
    win.flip()

    print(f"Spatial Trial {trial + 1}: Word='{word}', Position='{position_name}'")

    start_time = core.getTime()
    keys = event.waitKeys(keyList=['t', 'f', 'escape'])
    rt = core.getTime() - start_time

    if 'escape' in keys:
        print("👋 Task exited early by user.")
        win.close()
        core.quit()

    response = keys[0].lower()
    match = (word.lower() == position_name)
    correct = (response == 't' and match) or (response == 'f' and not match)

    spatial_results.append([
        trial + 1, word, position_name, response, correct,
        round(rt * 1000), participant_name, group, timestamp
    ])

    feedback.text = "Correct!" if correct else "Wrong!"
    feedback.pos = (0, 0)
    feedback.draw()
    win.flip()
    core.wait(0.5)

# End screen for Part 2
end_message.text = "Done with Part 2! Thank you.\nPress any key to continue to Part 3."
end_message.draw()
win.flip()
event.clearEvents()
wait_for_key()

# === Save Spatial Stroop results ===
with open(filename, 'a', newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow([])
    writer.writerow(['=== Spatial Stroop Task Results ==='])
    writer.writerow(['Trial', 'Word', 'Position_Shown', 'Response', 'Correct', 'Reaction_Time_ms', 'Participant', 'Group', 'Timestamp'])
    writer.writerows(spatial_results)

    total_trials = len(spatial_results)
    correct_trials = sum(1 for r in spatial_results if r[4])
    accuracy = (correct_trials / total_trials) * 100 if total_trials > 0 else 0
    avg_rt_correct = sum(r[5] for r in spatial_results if r[4]) / correct_trials if correct_trials > 0 else 0

    writer.writerow([])
    writer.writerow(['Summary'])
    writer.writerow(['Percent Accuracy (%)', f"{accuracy:.2f}"])
    writer.writerow(['Average RT (ms) Correct Answers', f"{avg_rt_correct:.0f}"])

print(f"✅ Spatial Stroop results appended to {filename}")

# === Part 3: Directional Stroop using ASCII arrows ===
instruction.text = (
    "PART 3: Directional Stroop\n\n"
    "An ASCII arrow with a word below (LEFT or RIGHT) will appear.\n"
    "Respond to the DIRECTION of the ARROW, ignoring the word.\n\n"
    "Press 'T' if the arrow points LEFT.\n"
    "Press 'F' if the arrow points RIGHT.\n\n"
    "Press any key to begin."
)
instruction.draw()
win.flip()
event.clearEvents()
wait_for_key()

directional_results = []

arrow_shapes = {
    'left': '<--',
    'right': '-->'
}

arrow_positions = (0, 50)
label_positions = (0, -50)

arrow_font_size = 80
label_font_size = 40

for trial in range(10):
    arrow_dir = random.choice(['left', 'right'])      # direction of the arrow
    word_label = random.choice(['LEFT', 'RIGHT'])     # distractor word
    arrow_symbol = arrow_shapes[arrow_dir]

    # Create stimuli
    arrow_stim = visual.TextStim(
        win=win, text=arrow_symbol, pos=arrow_positions,
        height=arrow_font_size, color='black'
    )
    label_stim = visual.TextStim(
        win=win, text=word_label, pos=label_positions,
        height=label_font_size, color='black'
    )

    # Draw both stimuli
    arrow_stim.draw()
    label_stim.draw()
    win.flip()

    print(f"Directional Trial {trial + 1}: Arrow='{arrow_dir}', Word='{word_label}'")

    start_time = core.getTime()
    keys = event.waitKeys(keyList=['t', 'f', 'escape'])
    rt = core.getTime() - start_time

    if 'escape' in keys:
        print("👋 Task exited early by user.")
        win.close()
        core.quit()

    response = keys[0].lower()
    correct = (response == 't' and arrow_dir == 'left') or (response == 'f' and arrow_dir == 'right')

    directional_results.append([
        trial + 1,
        arrow_dir,
        word_label,
        response,
        correct,
        round(rt * 1000),
        participant_name,
        group,
        timestamp
    ])

    feedback.text = "Correct!" if correct else "Wrong!"
    feedback.pos = (0, 0)
    feedback.draw()
    win.flip()
    core.wait(0.5)

# End screen for Part 3
end_message.text = "Done with Part 3! You completed all tasks.\nPress any key to exit."
end_message.draw()
win.flip()
event.clearEvents()
wait_for_key()
win.close()

# === Save Directional Stroop results ===
with open(filename, 'a', newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow([])
    writer.writerow(['=== Directional Stroop Task Results ==='])
    writer.writerow(['Trial', 'Arrow_Direction', 'Word_Label', 'Response', 'Correct', 'Reaction_Time_ms', 'Participant', 'Group', 'Timestamp'])
    writer.writerows(directional_results)

    total_trials = len(directional_results)
    correct_trials = sum(1 for r in directional_results if r[4])
    accuracy = (correct_trials / total_trials) * 100 if total_trials > 0 else 0
    avg_rt_correct = sum(r[5] for r in directional_results if r[4]) / correct_trials if correct_trials > 0 else 0

    writer.writerow([])
    writer.writerow(['Summary'])
    writer.writerow(['Percent Accuracy (%)', f"{accuracy:.2f}"])
    writer.writerow(['Average RT (ms) Correct Answers', f"{avg_rt_correct:.0f}"])

print(f"✅ Directional Stroop results appended to {filename}")

